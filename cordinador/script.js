...
let profesores = ['Adriana Sotelo', 'Mauricio Foronda', 'Luis Sanchez', 'Edison Marin'];
...
async function enviar() {
    ...
    horasNoDadas = parseInt(horasNoDadas.value);

    if (nombreDocente.value == 'Todos') {
        for (let i = 0; i < profesores.length; i++){
            nombreDocente = profesores[i];
            const {data, error} = await client 
                .from('prueba')
                .insert([{nombre: nombreDocente, horas: horasFinal, motivo: motivo.value, fecha: fechaFinal}]);
            if (error) {
        ...
        const {data, error} = await client 
            .from('prueba')
            .insert([{nombre: nombreDocente.value, horas: horasNoDadas, motivo: motivo.value, fecha: fecha}]);
